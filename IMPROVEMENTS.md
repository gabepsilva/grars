...
- [x] Replace clipboard operations with `arboard` crate:
  ...
  - [ ] Remove `xclip`/`wl-copy` shell commands (Linux)
  - [x] Update `src/system/clipboard.rs` to use `arboard` (macOS only)
- [ ] Replace URL opening with `open` crate:
  - [ ] Remove `Command::new("open")` (macOS)
  - [ ] Remove `Command::new("xdg-open")` (Linux)
  - [ ] Remove `Command::new("cmd /c start")` (Windows)
  - [ ] Update `open_url()` function in `src/update.rs`
